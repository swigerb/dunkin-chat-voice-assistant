...
- **Rate-limit recovery**: The Azure OpenAI quota is shared, so a response can be rate-limited. Instead of going silent, the middle tier retries it once quietly; if that is rate-limited too, the browser plays a pre-recorded "Sorry, give me just a second." clip in the UI language (en/es/fr/ja, mic muted while it plays) and the middle tier retries once more; after that the guest is asked to say it again. A pending retry is dropped as soon as the guest speaks. Settings live under `resilience.rate_limit` in `app/backend/config.yaml` (`RATE_LIMIT_RECOVERY_ENABLED` overrides `enabled`). The clips are in `app/frontend/public/audio/` and can be regenerated with `python scripts/generate_apology_clips.py` (e.g. after changing the default voice).
- **Voice picker**: The settings dialog exposes the ten gpt-realtime-2.1 voices (marin and cedar, which OpenAI recommends, then alloy, ash, ballad, coral, echo, sage, shimmer, verse) with short descriptors. The default is `marin` (`model.default_voice` in `app/backend/config.yaml`, overridden by `AZURE_OPENAI_REALTIME_VOICE_CHOICE`). The choice is persisted in the browser and sent to the middle tier with no redeploy. Before the assistant has spoken it is applied immediately; after that the realtime service locks the voice for the conversation, so the change applies from the next conversation, i.e. the next page load (sending it anyway would get the whole `session.update`, tools included, rejected).
