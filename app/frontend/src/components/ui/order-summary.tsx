...
export default function OrderSummary({ order }: { order: OrderSummaryProps }) {
    ...
    return (
        <div className="rounded-3xl border border-[#FF9F45]/30 bg-linear-to-br from-white via-[#FFF6EF] to-[#FFEFFC] p-5 shadow-[0_20px_45px_rgba(255,103,31,0.12)] dark:border-white/15 dark:bg-linear-to-br dark:from-[#1b1418] dark:via-[#231925] dark:to-[#1b1418]">
            ...
            <div className={`space-y-2 ${isExpanded ? "block" : "hidden md:block"}`}>
                ...
                {items.map((item, index) => (
                    <div key={index} className="flex justify-between rounded-2xl bg-white/70 px-3 py-2 text-sm text-gray-700 shadow-xs dark:bg-white/5 dark:text-white">
