...
function CoffeeApp() {
    ...
    // short grace period. Anything else (idle 4000, superseded 4002, rejected
    // resume, retries exhausted) means the order is gone and the next tap starts fresh.
    ...
    const realtime = useRealTime({
        ...
        onReceivedSessionResumed: (message: ExtensionSessionResumed) => {
            // Same shape as an update_order result: the ticket comes back exactly as it was.
            ...
            if (isSessionActiveRef.current) {
                ...
                // on, and no greeting is coming to end the tap's wait for one.
                if (awaitingGreetingDoneRef.current) {
    ...
    // Voice/VAD come back via set_voice + session.update (the server suppresses
    // the greeting), and the mic restarts without a tap when the browser allows it.
